...
#__EOF
  ...
  config.vm.define 'adm', primary: true do |adm|
    adm.vm.hostname = 'SOE-IML.local'
    adm.disksize.size = '65GB'
  ...
  (1..2).each do |i|
    config.vm.define "mds#{i}" do |mds|
      mds.vm.hostname = "SOE-DS-ARCH-MDS#{i}.local"
      ...
      if i == 1
        mds.vm.provision 'create-pools',
                         ...
                         inline: <<-SHELL
                           genhostid
                           zpool create mgtpool -O canmount=off -o multihost=on mpatha
                           zpool create mdtpool -O canmount=off -o multihost=on raidz mpathb mpathc mpathd
                         SHELL
  ...
  (1..2).each do |i|
    config.vm.define "oss#{i}",
      ...
      oss.vm.hostname = "SOE-DS-ARCH-OSS#{i}.local"
      ...
      if i == 1
        oss.vm.provision 'create-pools',
                         ...
                         inline: <<-SHELL
                           genhostid
                           zpool create ostpool0 -O canmount=off -o multihost=on mpatha mpathb mpathc mpathd mpathe mpathf mpathg mpathh
                           zpool create ostpool1 -O canmount=off -o multihost=on mpathi mpathj mpathk mpathl mpathm mpathn mpatho mpathp
			   zpool add ostpool0 spare mpathq mpathr
			   zpool add ostpool1 spare mpaths mpatht
                         SHELL
